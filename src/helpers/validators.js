...
export const checkWithdrawAmount = (available, min) => value => {
  const inputCents = value * 100;
  let tip;
  if (isNaN(inputCents) || inputCents < min) {
    tip = '请输入正确的取现金额';
  } else if (inputCents > MAX_WITHDRAW) {
    tip = '最多取现5万元';
  } else if (available < inputCents) {
    tip = '可取现金额不足';
  }
  return tip;
};
